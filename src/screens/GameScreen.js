...
const GameScreen = ({ isHermitMode, onBackToHome }) => {
  ...
  const saveHistory = () => {
    const currentState = {
      ...
      currentPlayer, gameStatus, winner, turnCount, movesInCurrentTurn,
  ...
  const validateCandidate = (newCandidate, player, currentLockedSets) => {
    ...
    const isExactMatch = mySets.some(set => 
      JSON.stringify(set.cells.sort()) === JSON.stringify(newCandidate.cells.sort())
    );
    ...
    const counts = {};
    allCells.forEach(c => { counts[c] = (counts[c] || 0) + 1; });
      ...
      <div style={{
        ...
        <div style={styles.header}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
             <div>
               <h1 style={{...styles.title, color: isHermitMode ? COLORS.hermitText : COLORS.text}}>
                 {isHermitMode ? '仙人対局' : '二人対局'}
               </h1>
             </div>
             <button onClick={onBackToHome} style={styles.homeBtn}>⌂ 戻る</button>
          </div>
        ...
        <div style={styles.controls}>
          {gameStatus === STATUS.SELECTING ? (
            <div style={styles.selectionUI}>
              <div style={styles.selectionMsg}>
                {candidates.length > 1 ? `選択せよ (${selectedCandidateIndex + 1}/${candidates.length})` : "確定中..."}
              </div>
              {candidates.length > 1 && (
                <div style={styles.selectionButtons}>
                  {candidates.map((_, idx) => (
                    <button key={idx} style={{...styles.optionBtn, backgroundColor: idx === selectedCandidateIndex ? COLORS.selected : '#ddd', color: idx === selectedCandidateIndex ? '#fff' : '#333'}} onClick={() => setSelectedCandidateIndex(idx)}>{idx + 1}</button>
                  ))}
                </div>
              )}
              <button style={styles.confirmBtn} onClick={handleConfirmClick}>{candidates.length > 1 ? "此れで確定" : "確定"}</button>
            </div>
          ) : (
             <div style={styles.bottomButtons}>
               <button onClick={handleUndo} disabled={history.length === 0} style={{...styles.undoButton, opacity: history.length === 0 ? 0.5 : 1, flex: 1}}>↺ 待った</button>
               <button onClick={() => setShowInGameRules(true)} style={{...styles.ruleBtn, flex: 1}}>? 指南書</button>
             </div>
          )}
        </div>
      </div>
...
const styles = {
  gameCard: {
    borderRadius: '8px', boxShadow: '0 10px 30px rgba(0,0,0,0.15)',
    display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', maxWidth: '420px', padding: '15px 10px', position: 'relative',
  },
  header: { width: '100%', textAlign: 'center', marginBottom: '10px' },
  title: { fontSize: '20px', margin: '0', fontWeight: 'bold' },
  homeBtn: { padding: '5px 12px', fontSize: '12px', border: `1px solid ${COLORS.border}`, borderRadius: '4px', background: '#fff', cursor: 'pointer', fontFamily: 'inherit' },
  infoRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 10px', marginTop: '10px' },
  playerBadge: { padding: '5px 10px', borderRadius: '20px', color: 'white', fontSize: '13px', fontWeight: 'bold', minWidth: '60px', textAlign: 'center', boxShadow: '0 2px 4px rgba(0,0,0,0.2)' },
  score: { display: 'inline-block', backgroundColor: 'rgba(255,255,255,0.3)', borderRadius: '50%', width: '18px', height: '18px', lineHeight: '18px', fontSize: '11px', marginLeft: '4px' },
  ...
  cell: { 
    ...
    display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', touchAction: 'manipulation' 
  },
  ...
  confirmBtn: { width: '100%', padding: '12px', backgroundColor: COLORS.selected, color: '#fff', border: 'none', borderRadius: '4px', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' },
  bottomButtons: { display: 'flex', gap: '10px', width: '100%' },
  undoButton: { padding: '12px', fontSize: '14px', backgroundColor: '#555', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontFamily: 'inherit' },
  ruleBtn: { padding: '12px', fontSize: '14px', backgroundColor: COLORS.text, color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontFamily: 'inherit' },
};
